...
fn queue_subcommands_parse() {
    ...
    let args = Args::try_parse_from(["jcode", "queue", "init", "--force"]).unwrap();
    match args.command {
        Some(Command::Queue(QueueCommand::Init { force })) => {
            assert!(force);
        }
        other => panic!("unexpected command: {:?}", other),
    }

    let args = Args::try_parse_from(["jcode", "queue", "workers"]).unwrap();
    match args.command {
        Some(Command::Queue(QueueCommand::Workers)) => {}
        other => panic!("unexpected command: {:?}", other),
    }

    let args = Args::try_parse_from(["jcode", "queue", "worker", "coder"]).unwrap();
    match args.command {
        Some(Command::Queue(QueueCommand::Worker { name })) => {
    ...
    .unwrap();
    match args.command {
        Some(Command::Queue(QueueCommand::Runs { task_id, limit })) => {
            assert_eq!(task_id.as_deref(), Some("task_1"));
            assert_eq!(limit, 5);
        }
        other => panic!("unexpected command: {:?}", other),
    }
    ...
    .unwrap();
    match args.command {
        Some(Command::Queue(QueueCommand::Run {
            task_id,
            timestamp,
            stdout,
            stderr,
            ...
            assert_eq!(task_id, "task_1");
            assert_eq!(timestamp, "20260620T100000Z");
            assert!(stdout);
            assert!(stderr);
        }
        other => panic!("unexpected command: {:?}", other),
